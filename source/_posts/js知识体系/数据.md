...
parseInt('070')          // 70  字符串类型会忽略前置零。
parseInt(070)            // 56  符合二进制按二进制转换
parseInt('70')           // 70
parseInt('0b1100')       // 0
parseInt(0B1100)         // 12
```
...
[[get]] 读取属性时调用的函数,[[set]] 给属性设置值的时候调用的函数
...
## Date
  ...
  ### 时区
    ...
    javaScript 中，我们只关心两个，本地时间和协调世界时(UTC)
      - 本地时间 是指自己计算机所在的时区
      - UTC 实际上是格林威治时间
  ### 创建
    `new Date()` 来创建日期，传入的参数通常有四种
    - 使用日期字符串参数
    - 使用一系列的参数
      可以传入七个参数来创建日期和时间
      - Year: 四位数年份
...
### 日期提供的方法
  ...
  - 编写自定义日期格式化
  Date 中获取 年月日时分秒的方法
    - getFullYear: 获取当地时间四位数的年份
    - getMonth: 获取月份，月份从零开始
    - getDate: 获取月中的某一天(1-31)
    - getDay: 获取星期几(0-6)，星期日(0)开始，到星期六(6)结束。
    - getHours: 获取小时数(0-23)
    - getMinutes: 获取分钟数(0-59)
    - getSeconds: 获取秒数(0-59)
    - getMilliseconds: 获取毫秒数(0-999)
  
  Date 设置另一个日期特定的日期/时间值
    - setFullYear: 设置年份
    - setMonth: 设置月份
    - setDate: 设置每月中的某一天
    - setHours: 设置时
...
#### 日期添加/减去增量
  ...
  ```
    当前日期 2019年3月28日，获取三天后的日期

    let today = new Date(2019,2,28)
